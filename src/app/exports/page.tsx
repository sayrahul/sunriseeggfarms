...
import { Globe2, Mail, Phone, Plane, Ship } from "lucide-react";
  ...
  return (
    <div className="py-24 max-w-7xl mx-auto px-4 md:px-6 relative overflow-hidden">
      ...
      <AnimatedSection className="text-center mb-16 max-w-3xl mx-auto">
        ...
        <h1 className="text-4xl md:text-5xl font-bold text-slate-dark mb-6">Global <span className="text-brand">Exports</span></h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <AnimatedSection delay={0.4} className="relative h-[400px] bg-slate-dark rounded-3xl overflow-hidden shadow-2xl flex items-center justify-center">
          {/* Animated Globe Pseudo-Representation */}
          <div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'radial-gradient(circle at center, #F59E0B 0%, transparent 70%)' }}></div>
          
          <div className="relative w-64 h-64 border-[0.5px] border-white/20 rounded-full flex items-center justify-center">
            <div className="absolute w-48 h-48 border-[0.5px] border-white/20 border-dashed rounded-full animate-spin-slow"></div>
            <motion.div 
              animate={{ rotate: 360 }} 
              transition={{ repeat: Infinity, duration: 20, ease: "linear" }}
              className="absolute w-full h-full"
            >
              <Plane className="absolute -top-3 left-1/2 text-white w-6 h-6 -ml-3 rotate-45" />
            </motion.div>
            <motion.div 
              animate={{ rotate: -360 }} 
            ...
            >
              <Ship className="absolute bottom-0 right-0 text-brand w-6 h-6" />
            </motion.div>
            <span className="font-bold text-white tracking-widest uppercase opacity-50">Global</span>
          </div>
        </AnimatedSection>
      </div>
    </div>
